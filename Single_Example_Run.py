...
wtest = I1/fs.evperAU-0.5/4.45**2
...
# position of CM15 state, used as a reference to compute energy dependences.
u2 = 26.84082546
...
e_axis = np.linspace(24.5,25.6,150)
delays = np.linspace(50/fs.fsperau,3*np.pi/0.05 * fs.evperAU,150)
...
param_dict = {
        ...
        'w' : 0.75 / fs.evperAU, #0.93/fs.evperAU, #0.88/ fs.evperAU, #0.85
        'wuv' : wtest, #26.95 / fs.evperAU,
        'per' : 0.05 / fs.evperAU,
...
peaks_s1, props1 = sg.find_peaks(np.gradient(np.real(phs_S1),(erange[1]-erange[0])/fs.evperAU))
peaks_s2, props2 = sg.find_peaks(np.gradient(np.real(phs_S2),(erange[1]-erange[0])/fs.evperAU))
...
def main():
    ...
    fs.compute_view_parallel(coefs1, coefs2, Deigen, e_axis, param_dict)
    
    np.save("peaks_S1.npy", peaks_s1)
    np.save("peaks_S2.npy", peaks_s2)
