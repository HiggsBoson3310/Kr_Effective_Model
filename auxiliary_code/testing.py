...
fig,ax = plt.subplots()
nns = [mqdt.nu(erange/fs.evperAU,I1/fs.evperAU),
       mqdt.nu(erange/fs.evperAU,I2/fs.evperAU)]
for c,q in zip(coefs1[:2],nns):
    ax.plot(erange, np.abs(c(erange/fs.evperAU))**2)

for s in state_loc_1:
    ax.axvline(s)
    
ax.set_xlim(26.5,27)
ax.set_title('$|C|^2$ coefficients')
ax.set_xlabel("Energy eV")
plt.savefig('./Figures/c_coeffs_interp.png')
plt.close()


fig,ax = plt.subplots()

ax.plot(erange, np.abs(Z1_1))
ax.plot(erange, np.abs(Z1_2))
...
ax.set_xlim(26.5,27)
#ax.set_yscale('log')
ax.set_title('$|Z|^2$ coefficients')
ax.set_xlabel("Energy eV")
plt.savefig('./Figures/Z_coeffs.png')
STOP
